      ...
      <div className="border-b border-surface-container/20 bg-surface-container-low/10 sticky top-0 z-20 backdrop-blur-md">
        <Container>
          <div className="flex items-center gap-3 py-5">
            <Link href="/jobs" className="text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant hover:text-primary transition-colors">Operational Inventory</Link>
            <span className="material-symbols-outlined text-sm text-outline/30">chevron_right</span>
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary truncate">Slot: {job.title}</span>
          </div>
        </Container>
      </div>
          ...
          <section className="space-y-16">
              ...
              <div className="flex flex-wrap gap-10">
                <div className="flex items-center gap-3 group">
                   ...
                   <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest opacity-40">Deployment Date</span>
                      <span className="text-sm font-black text-on-surface">{new Date(job.createdAt).toLocaleDateString()}</span>
                   </div>
                </div>
                <div className="flex items-center gap-3 group">
                   <div className="w-10 h-10 rounded-xl bg-surface-container-low flex items-center justify-center text-primary shadow-sm transition-transform group-hover:scale-110">
                   ...
                   <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest opacity-40">Operational Zone</span>
                      <span className="text-sm font-black text-on-surface">Universal Remote / Agnostic</span>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-10">
              <div className="flex items-center gap-4">
                 <h2 className="font-headline text-3xl font-black tracking-tight text-on-surface uppercase">Scope & Requirements</h2>
          ...
          <aside className="space-y-10">
            <div className="sticky top-32 p-10 bg-surface-container-lowest border border-surface-container/20 rounded-[2.5rem] shadow-2xl shadow-black/[0.02]">
              <div className="flex items-center gap-3 mb-10 pb-6 border-b border-surface-container/30">
                 ...
                 <h3 className="font-headline text-xl font-black uppercase tracking-tight text-on-surface">Slot Calibrations</h3>
              </div>
              ...
              <div className="space-y-10">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {skills.map((skill: string) => (
                      <span key={skill} className="px-4 py-2 bg-surface-container-low text-on-surface text-[10px] font-black uppercase tracking-widest rounded-xl border border-surface-container/30 transition-all hover:bg-primary/10 hover:text-primary hover:border-primary/20">
                        {skill}
                      </span>
                    ))}
                  </div>
                ...
                <div className="pt-10 border-t border-surface-container/30 space-y-6">
                  <Link href={`/apply/${job.slug}`}>
                    <button className="w-full btn-gradient py-6 rounded-2xl text-on-primary font-headline font-black uppercase tracking-[0.2em] text-xs soft-scale shadow-2xl shadow-primary/30 flex items-center justify-center gap-3">
                  ...
                  <p className="text-[9px] font-black text-on-surface-variant/40 text-center uppercase tracking-widest leading-relaxed">
                    Neural evaluation results typically transmitted within 48 operational hours
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-8 bg-secondary/5 border border-secondary/10 rounded-[2rem] flex flex-col items-center text-center gap-4 group">
              ...
              <div className="space-y-1">
                <h4 className="text-xs font-black text-secondary uppercase tracking-[0.1em]">Protocol Referrals</h4>
                <p className="text-[11px] font-medium text-on-surface-variant opacity-60 px-4">
                  Identify and synchronize high-performance peers with this operational slot.
                </p>
              </div>
            </div>
          </aside>
